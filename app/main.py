...
# Global variable for polling task
polling_task = None
...
async def process_greenapi_message(data: dict):
    """Process Green-API message (webhook or polling)"""
...
async def greenapi_polling_task():
    """Background task to poll Green-API for messages"""
    ...
    while True:
        try:
            if greenapi_handler.enabled:
                ...
                if notification:
                    app_logger.info("📨 Processing polled notification")
                    await process_greenapi_message(notification)
            
            # Esperar 2 segundos antes de la próxima consulta
            await asyncio.sleep(2)
        
        except Exception as e:
            app_logger.error(f"Error in polling task: {e}")
...
async def lifespan(app: FastAPI):
    ...
    # Start polling task if Green-API is enabled
    if greenapi_handler.enabled:
        polling_task = asyncio.create_task(greenapi_polling_task())
        app_logger.info("✅ Green-API polling started")
    else:
        app_logger.warning("⚠️ Green-API not enabled, polling not started")
    ...
    if polling_task:
        polling_task.cancel()
        app_logger.info("🛑 Green-API polling stopped")
...
async def health_check():
    ...
    return {
        ...
        "services": {
            ...
            "greenapi": greenapi_handler.enabled,
...
# ==========================================
# GREEN-API WEBHOOK (BACKUP)
# ==========================================

@app.post("/webhook/greenapi")
async def greenapi_webhook(request: Request):
    """Green-API webhook endpoint (backup for polling)"""
    try:
        data = await request.json()
        await process_greenapi_message(data)
        return Response(content="", status_code=200)
    except Exception as e:
